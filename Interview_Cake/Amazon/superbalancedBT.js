...
//* My solution: - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// Time: O(n) - iterate through all nodes worst case
// Space: O(n) - queue may have to hold all the nodes
var isSuperbalanced = function(root) {
    // edge case: if tree has no nodes, then it is superbalanced
    if (root === null) return true;

    const depths = [];           // should hold no more than two depths
    const queue = [[root, 0]];   // holds the nodes to iterate on next

    while (queue.length > 0) {
        let [node, depth] = queue.pop();    // pop faster than shift

        // check if current node is a leaf (meaning no children)
        if (node.left === null && node.right === null) {
            if (!depths.include(depth)) {
                depths.push(depth);
                // two cases where tree is not superbalanced:
                // 1) more than two depths
                if (depths.length > 2) return false;
                // 2) the two depths differ by more than 1
                if (depths.length === 2) {
                    if (Math.abs(depths[1] - depths[0]) > 1) return false;
                }
            }
        }
        // insert more nodes into queue to check if they are leaves
